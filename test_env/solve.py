...
# Ensure your project root (containing env/) is on PYTHONPATH
...
def run_old_solve(env, max_steps=200):
    # Personality tweaks for agents 0,1,2
    env.agent_params[0].update({
        'min_energy_to_speak':0.6, 'energy_gain':0.01,
        'energy_decay':0.15, 'max_speaking_time':2,
        'phonemes_per_step':1
    })
    env.agent_params[1].update({
        'min_energy_to_speak':0.3, 'energy_gain':0.05,
        'energy_decay':0.10, 'max_speaking_time':5,
        'phonemes_per_step':2
    })
    env.agent_params[2].update({
        'min_energy_to_speak':0.1, 'energy_gain':0.10,
        'energy_decay':0.05, 'max_speaking_time':8,
        'phonemes_per_step':4
    })
...
def run_new_solve(env, max_steps=200, window=20, threshold=15):
    ...
    cooldowns = np.zeros(len(env.agent_params), int)

    obs, info = reset_env(env, seed=0)
    ph_hist, g_hist = [], []
    for _ in range(max_steps):
        # Compute windowed phoneme counts
        wc = np.sum(dq, axis=0) if dq else np.zeros(len(env.agent_params), int)
        quiet = int(np.argmin(wc))
        loud  = int(np.argmax(wc))

        # If loudness gap is too large, stare at loudest
        if wc[loud] - wc[quiet] > threshold:
            action = 2 + loud       # actions 2,3,4 = stare at agent 0,1,2
        elif cooldowns[quiet] > 0:
            action = 0              # wait
        else:
            action = 5 + quiet      # encourage quietest
            cooldowns[quiet] = window

        obs, reward, done, info = step_env(env, action)
        phonemes = np.array(info.get("phoneme", [0]*len(env.agent_params)), int)
        ph_hist.append(phonemes)
        g_hist.append(1.0 - info.get("env_reward", 0))
...
if __name__ == "__main__":
    ...
    # Improved heuristic run
    env3 = GuestEnv()
    n_ph, n_g = run_new_solve(env3, STEPS)
